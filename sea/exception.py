...
Includes decorator for re-raising exceptions.
...
from sea.utils import safe as safe_utils
...
class EncryptionFailure(BaseException):
...
class VirtualInterfaceCreateException(BaseException):
    msg_fmt = _("Virtual Interface creation failed")


class VirtualInterfaceMacAddressException(BaseException):
    msg_fmt = _("5 attempts to create virtual interface"
                "with unique mac address failed")


class GlanceConnectionFailed(BaseException):
    msg_fmt = _("Connection to glance host %(host)s:%(port)s failed: %(reason)s")
...
class ImageNotActive(BaseException):
...
class ImageNotAuthorized(BaseException):
...
class VolumeNotCreated(BaseException):
...
class ConstraintNotMet(BaseException):
...
class AgentBuildExists(BaseException):
...
class NetworkInUse(BaseException):
    msg_fmt = _("Network %(network_id)s is still in use.")


class NetworkNotCreated(BaseException):
...
class FixedIpExists(BaseException):
...
class FixedIpAlreadyInUse(BaseException):
    msg_fmt = _("Fixed IP address %(address)s is already in use on instance "
                "%(instance_uuid)s.")


class FixedIpAssociatedWithMultipleInstances(BaseException):
...
class NoMoreFixedIps(BaseException):
    ec2_code = 'UnsupportedOperation'
...
class FloatingIpExists(BaseException):
...
class FloatingIpMultipleFoundForAddress(BaseException):
...
class FloatingIpAssociated(BaseException):
    ec2_code = "UnsupportedOpperation"
    msg_fmt = _("Floating ip %(address)s is associated.")


class FloatingIpNotAssociated(BaseException):
...
class CannotDisassociateAutoAssignedFloatingIP(BaseException):
    ec2_code = "UnsupportedOpperation"
...
class ServiceBinaryExists(BaseException):
    msg_fmt = _("Service with host %(host)s binary %(binary)s exists.")


class ServiceTopicExists(BaseException):
...
class QuotaExists(BaseException):
...
class ConsolePoolExists(BaseException):
...
class CellExists(BaseException):
    msg_fmt = _("Cell with name %(name)s already exists.")


class CellRoutingInconsistency(BaseException):
...
class QuotaError(BaseException):
...
class AggregateError(BaseException):
...
class AggregateNameExists(BaseException):
...
class CouldNotFetchImage(BaseException):
    msg_fmt = _("Could not fetch image %(image_id)s")


class CouldNotUploadImage(BaseException):
    msg_fmt = _("Could not upload image %(image_id)s")


class TaskAlreadyRunning(BaseException):
    msg_fmt = _("Task %(task_name)s is already running on host %(host)s")


class TaskNotRunning(BaseException):
...
class ConfigDriveMountFailed(BaseException):
    msg_fmt = _("Could not mount vfat config drive. %(operation)s failed. "
                "Error: %(error)s")


class ConfigDriveUnknownFormat(BaseException):
...
class ServiceGroupUnavailable(BaseException):
    msg_fmt = _("The service from servicegroup driver %(driver)s is "
                "temporarily unavailable.")


class DBNotAllowed(BaseException):
...
class UnsupportedObjectError(BaseException):
    msg_fmt = _('Unsupported object type %(objtype)s')


class OrphanedObjectError(BaseException):
    msg_fmt = _('Cannot call %(method)s on orphaned %(objtype)s object')


class IncompatibleObjectVersion(BaseException):
    msg_fmt = _('Version %(objver)s of %(objname)s is not supported')


class ObjectActionError(BaseException):
...
class InstanceGroupIdExists(BaseException):
...
class PluginRetriesExceeded(BaseException):
    msg_fmt = _("Number of retries to plugin (%(num_retries)d) exceeded.")


class ImageDownloadModuleError(BaseException):
...
class PciDeviceWrongAddressFormat(BaseException):
...
class PciTrackerInvalidNodeId(BaseException):
